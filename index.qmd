...
From the summary, I can see that the Quantity column has no missing values and most values are small. The mean is about 2.23, which shows that most cookie ingredients are used in relatively small amounts.
...
top20_table = (
    ...
    # Keep only unique recipe–ingredient pairs to avoid double counting
    .drop_duplicates()
...
From this plot, I can see that ingredients like eggs, vanilla, and flour appear in the largest number of recipes. This suggests that these ingredients are very common and are basic components in cookie recipes. In the future, it would be interesting to explore how ingredient usage differs across different types of cookies.